...
class PlotWindow(QMainWindow):
    ...
    def _on_table_context_menu(self, pos) -> None:
        ...
        menu = QMenu(self)
        select_all_action = menu.addAction("Select All parameters")
        unselect_all_action = menu.addAction("Unselect all parameters")
        menu.addSeparator()
        ...
        if chosen is unload_action:
            ...
            self._set_all_traces_for_file(fid, True)
        elif chosen is unselect_all_action:
            self._set_all_traces_for_file(fid, False)

    def _set_all_traces_for_file(self, file_id: str, select: bool) -> None:
        loaded = next(
            (item for item in self._state.get_loaded_files() if item.file_id == file_id),
            None,
        )
        if loaded is None:
            return
        if select:
            self._selected_traces[file_id] = set(loaded.data.trace_names)
        else:
            self._selected_traces[file_id] = set()
        self.refresh_from_state()
        self.project_modified.emit()
