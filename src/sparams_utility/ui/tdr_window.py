...
class TdrWindow(QMainWindow):
    ...
    def __init__(self, state: AppState, parent=None, window_number: int = 1) -> None:
        ...
        # Use the dedicated pulse icon (matches the project tree entry).
        try:
            import importlib.resources as _pkg_resources
            from PySide6.QtGui import QIcon as _QIcon
            import sparams_utility.resources as _res_pkg

            _resource = _pkg_resources.files(_res_pkg).joinpath("pulse_icon.svg")
            with _pkg_resources.as_file(_resource) as _path:
                _pulse_icon = _QIcon(str(_path))
        except Exception:
            _pulse_icon = None
        if _pulse_icon is not None and not _pulse_icon.isNull():
            self.setWindowIcon(_pulse_icon)
        ...
        self._window_combo.setCurrentText("None")
